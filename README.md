...
安装包和 zip **不要**提交进 git，只放到 [Releases](https://github.com/alanjun0411/dsh-community-desktop/releases)。

1. 在 DeepSeek Harness 源码目录打包（国内建议带上 Electron 镜像）：

   ```powershell
   $env:DSH_DESKTOP_GITHUB_OWNER = 'alanjun0411'
   $env:DSH_DESKTOP_GITHUB_REPO = 'dsh-community-desktop'
   $env:ELECTRON_MIRROR = 'https://npmmirror.com/mirrors/electron/'
   $env:ELECTRON_BUILDER_BINARIES_MIRROR = 'https://npmmirror.com/mirrors/electron-builder-binaries/'
   pnpm run package:desktop:win:community
   ```

2. 打开 [新建 Release](https://github.com/alanjun0411/dsh-community-desktop/releases/new)。Tag 用 `v0.1.5-rc.1` 这种与 dsh 版本对应的名字。**不要勾 Pre-release**，这样才会成为 Latest。
3. 上传产物目录 `apps/desktop/.desktop-build/targets/win-x64/artifacts` 里的：
   - `dsh-community-desktop-<version>-win-x64.exe`
   - `dsh-community-desktop-<version>-win-x64.exe.blockmap`
   - `dsh-community-desktop-<version>-win-x64.zip`
   - `latest.yml` 或 electron-builder 生成的频道文件（预发布版本可能是 `rc.yml`）
4. Release 说明里写清改了什么。已安装用户点「检查更新…」时会看到这段说明。

安装包和 zip 只放 Releases，不要提交进 git。有 GitHub Token 时可以运行 `publish-docs.ps1` 只上传说明文件。
